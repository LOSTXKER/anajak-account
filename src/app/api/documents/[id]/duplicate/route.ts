...
export async function POST(
  ...
  try {
    ...
    const prefix = sourceDoc.documentType.code.toUpperCase()
    const year = new Date().getFullYear()
    const lastDoc = await prisma.document.findFirst({
      where: {
        companyId: tenantId,
        documentTypeId: sourceDoc.documentTypeId,
        documentNumber: {
    ...
    console.log('✅ Document duplicated:', sourceDoc.id, '→', newDoc.id)
    ...
    return NextResponse.json(
      { error: 'เกิดข้อผิดพลาดในการคัดลอกเอกสาร' },
      { status: 500 }
    )
  }
}
